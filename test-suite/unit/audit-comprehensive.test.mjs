      ...
      executeScript: async (opts) => {
        if (opts.func) {
          ...
          if (funcStr.length > 5000 || funcStr.includes("querySelectorAll(") || funcStr.includes("styleSheets") || funcStr.includes("isLandmark")) {
            ...
            mockAudit.imageAudit = { total: 2, missingDimensions: 0, missingAlt: 1, lazyLoaded: 0, withoutExplicitSize: 0, large: [] };
            mockAudit.typographyDetails = { lineHeights: [{ value: "24px", count: 5 }], letterSpacing: [] };
            mockAudit.colorFormats = { hex: 3, rgb: 0, rgba: 0, hsl: 0, hsla: 0, named: 0, other: 0 };
            mockAudit.flexGridAudit = { flexContainers: [], gridContainers: [] };
            mockAudit.hiddenContent = [];
            mockAudit.eventHandlers = { inlineCount: 0, attributes: [] };
            mockAudit.cssSize = { totalRules: 10, estimatedSizeChars: 2000, files: [] };
            mockAudit.buttonStyles = [];
            mockAudit.linkStyles = [];
            mockAudit.nestingDepth = [];
            mockAudit.headingHierarchy = { total: 3, issues: [] };
            mockAudit.overflowAudit = [];
            mockAudit.unusedClasses = { defined: 50, used: 45, unusedCount: 5, unused: [] };
            mockAudit.pointerEvents = [];
            mockAudit.borderAudit = [];
            mockAudit.opacityTransform = { opacityElements: [], transformElements: [] };
            mockAudit.responsiveImages = { total: 2, withSrcset: 1, withSizes: 0, inPicture: 0, missingSrcset: 1, missingSizes: 1 };
            mockAudit.containerQueries = { count: 0, samples: [] };
            mockAudit.printStyles = { hasPrintStyles: false, estimatedRules: 0 };
            mockAudit.willChange = [];
            mockAudit.focusStyles = { focusableCount: 5, withOutline: 3, withBoxShadowFocus: 1, noIndicator: 1 };
            mockAudit.hoverStates = { hoverRules: 3, hasFocusVisible: true, samples: [] };
            mockAudit.reducedMotion = { hasReducedMotion: false };
            mockAudit.readingWidth = [];
            mockAudit.brokenImages = [];
            mockAudit.specificity = [];
            mockAudit.mixedContent = { secure: true, issues: [] };
            mockAudit.resourceHints = { total: 0, hints: [] };
            mockAudit.textOverflow = [];
            mockAudit.userSelect = [];
            mockAudit.svgAudit = { inline: 1, external: 0, withAriaLabel: 1, withRole: 0, withTitle: 0, withoutAccess: 0 };
            mockAudit.scrollSnap = [];
            mockAudit.touchAction = [];
            mockAudit.filterEffects = [];
            mockAudit.objectFit = [];
            mockAudit.scrollbarGutter = { hasGutter: 0, stable: 0 };
            mockAudit.cascadeLayers = { count: 0, names: [] };
            mockAudit.minMaxConstraints = [];
            mockAudit.gapAudit = [];
            mockAudit.contrastRatios = [];
            mockAudit.customPropUsage = { totalUsages: 5, topProps: [] };
            mockAudit.viewportUnits = [];
            mockAudit.gridAreas = [];
            mockAudit.cursorAudit = { interactiveCursors: [], nonInteractivePointer: [] };
            mockAudit.textTransform = [];
            mockAudit.customScrollbars = { hasCustomScrollbar: false };
            mockAudit.selectionStyling = { hasSelectionStyling: false };
            mockAudit.stackingIsolation = { isolation: 0, backfaceHidden: 0 };
            mockAudit.textFlow = [];
            mockAudit.listStyles = { ulCount: 0, olCount: 0, dlCount: 0 };
            mockAudit.placeholderStyling = { hasPlaceholderStyling: false };
            mockAudit.tableStrategy = [];
            mockAudit.textRendering = { textRendering: [], fontSmoothing: [] };
            mockAudit.dataAttributes = [];
            mockAudit.resolutionAudit = { hasDppx: false, hasDpi: false, hasMinResolution: false };
            mockAudit.backgroundAudit = [];
            mockAudit.cssFunctions = [];
            mockAudit.preflightReset = { hasBoxSizing: true, hasMarginZero: true, hasBorderZero: true, likelyReset: true };
            mockAudit.accordionDisclosure = { detailsCount: 0, dialogCount: 0, popoverCount: 0 };
            mockAudit.imageFormat = { webp: 1, avif: 0, jpg: 0, png: 0, svg: 1, gif: 0 };
            mockAudit.counterContent = { hasCounter: false };
            mockAudit.whitespaceHandling = { cssGap: 3, brTags: 0, hrTags: 0, nbsp: 0 };
            mockAudit.performanceBudget = { totalResources: 2, totalSizeKB: 245, byType: {}, slowestRequests: [] };
            mockAudit.colorSchemes = { count: 0, schemes: [] };
            mockAudit.textColumns = [];
            mockAudit.outlineOffset = { withOffset: 1, withoutOffset: 2 };
            mockAudit.cssCustomSelect = { total: 0, customStyled: 0, native: 0 };
            mockAudit.subgridUsage = { count: 0, items: [] };
            mockAudit.langAndDir = { lang: "en", dir: "ltr", hasLang: true, isRtl: false };
            mockAudit.inputWidths = [];
            mockAudit.contentEditable = { count: 0 };
            mockAudit.prefersContrast = { hasPrefersContrast: false, hasForcedColors: false };
            mockAudit.shapeOutside = [];
            mockAudit.cssNesting = { hasNesting: false, nestedRules: 0 };
            mockAudit.fieldsetLegend = { fieldsetCount: 0, withLegend: 0, withoutLegend: 0 };
            mockAudit.audioVideo = { videos: 0, audios: 0 };
            mockAudit.skipLinks = { hasSkipLink: false };
            mockAudit.dupIds = [];
            mockAudit.emptyLinks = [];
            mockAudit.liveRegions = { polite: 0, assertive: 0, total: 0 };
            mockAudit.mathFunctions = [];
            mockAudit.anchorScrolling = { scrollBehavior: "smooth" };
            mockAudit.breadcrumbAudit = { hasBreadcrumbNav: false };
            mockAudit.titleHierarchy = { docTitle: "Test", firstH1: "Welcome" };
            mockAudit.duplicateTracking = { total: 0, services: [] };
            mockAudit.fontSwap = { hasFout: true, hasFoIT: false, hasSizeAdjust: false };
            mockAudit.cssCustomPropsFallback = { withFallback: 2, withoutFallback: 3 };
            mockAudit.inputWidthCols = [];
            mockAudit.headingDensity = { total: 3, estimatedScreens: 1 };
            mockAudit.listSemantics = { semanticLists: 0, divAsList: 0 };
            mockAudit.buttonVsLink = { linksAsButtons: 0 };
            mockAudit.fileInputAudit = [];
            mockAudit.regionLandmarks = [{ tag: "header", role: "banner", count: 1 }];
            mockAudit.colorScheme = { cssColorScheme: "light" };
            mockAudit.reducedData = { hasReducedData: false };
            mockAudit.propertyRegistration = { count: 1, props: ["--my-color"] };
            mockAudit.scrollDrivenAnimations = { count: 0, types: { scroll: 0, view: 0 } };
            mockAudit.autocapitalizeAudit = [];
            mockAudit.focusManagement = { autofocusElements: 0, inertElements: 0, hasSkipLink: false };
            mockAudit.viewportMetaDetailed = { found: true, content: "width=device-width", disablesZoom: false };
            mockAudit.cssNestingDepth = { maxDepth: 0, nestedCount: 0 };
            mockAudit.loadingStrategy = { images: { lazy: 0, eager: 0, none: 2 }, scripts: { async: 1, defer: 0, blocking: 1 } };
            mockAudit.trigFunctions = { total: 0, functions: [] };
            mockAudit.lineHeightRatio = [];
            mockAudit.letterSpacingAudit = [];
            mockAudit.visibleOverflow = [];
            mockAudit.gridItemAudit = [];
            mockAudit.cssAtRules = [];
            mockAudit.keyframeAnimations = { total: 1, names: ["fadeIn"] };
            mockAudit.widthCalcAudit = [];
            mockAudit.scrollDriven = { hasScrollTimeline: false, hasViewTimeline: false };
            mockAudit.namedGridLines = [];
            mockAudit.anchorPositioning = { count: 0 };
            mockAudit.fontFeatureSettings = [];
            mockAudit.mathStyle = { mathDepth: false, mathShift: false, mathVariant: false };
            mockAudit.conditionalRules = { supportsCount: 0, mediaCount: 2 };
            mockAudit.boxModelAudit = [];
            mockAudit.counterStyle = { count: 0, names: [] };
            mockAudit.initialLetter = [];
            mockAudit.cssSupports = [];
            mockAudit.textWrapAudit = [];
            mockAudit.lineClampAudit = [];
            mockAudit.rubyAnnotation = { rubyCount: 0, hasRuby: false };
            mockAudit.inheritViewport = { hasSvh: false, hasDvh: false, hasLvh: false };
            mockAudit.colorGamut = { hasSrgb: false, hasP3: false, hasRec2020: false };
            mockAudit.forcedColors = { hasForcedColors: false };
            mockAudit.contentVisibility = { count: 0, items: [] };
            mockAudit.textUnderlineOffset = [];
            mockAudit.gridAutoFlow = [];
            mockAudit.gapShorthand = { usesGap: 3, usesRowColGap: 0 };
            mockAudit.descendentSelectors = { count: 0, samples: [] };
            mockAudit.textOrientation = [];
            mockAudit.variableFonts = { count: 0, items: [] };
            mockAudit.reducedTransparency = { hasReducedTransparency: false };
            mockAudit.scopeRules = { count: 0, scopes: [] };
            mockAudit.fieldSizing = [];
            mockAudit.textSpacingTrim = { hasTextSpacingTrim: false };
            mockAudit.interpolateSize = { hasAllowKeywords: false, hasNumericOnly: false };
            mockAudit.fontPalette = { count: 0, palettes: [] };
            mockAudit.startingStyle = { count: 0 };
            mockAudit.scrollDrivenProgress = { count: 0, ranges: [] };
            mockAudit.directionAware = [];
            mockAudit.colorFunctionLevel = [];
            mockAudit.scrollTimelineNamed = { count: 0, names: [] };
            mockAudit.transitionProperties = [];
            mockAudit.originReporting = { hasOriginTrial: false, hasPermissionsPolicy: false };
            mockAudit.baselineAudit = { totalModernFeatures: 3, features: ["color-mix", "oklch", ":has()"] };
            mockAudit.hasSelectorAudit = { count: 0, samples: [] };
            mockAudit.isWhereSelectors = { isCount: 0, whereCount: 0, samples: [] };
            mockAudit.nestingCompatability = { hasNativeNesting: false };
            mockAudit.pageTransitionTypes = { count: 0, types: [] };
            mockAudit.positionTry = { count: 0, fallbacks: [] };
            mockAudit.cssBaselineStatus = { newly: 1, widely: 2, limited: 0 };
            mockAudit.inputEnterKeyHint = [];
            mockAudit.cssBorderImage = [];
            mockAudit.anchorInset = { count: 0 };
            mockAudit.textAutospace = { count: 0 };
            mockAudit.spreadGlyph = { cssCount: 0, dataAttrCount: 0 };
            mockAudit.crossOriginIsolation = { crossOriginIsolated: false, isSecureContext: true };
            mockAudit.paintWorklet = { paintWorklet: false, layoutWorklet: false, animationWorklet: false };
            mockAudit.prefersInstant = { hasPrefersInstant: false };
            mockAudit.intrinsicSizing = [];
            mockAudit.gridSpanAudit = [];
            mockAudit.icLhUnits = [];
            mockAudit.unknownAtRules = [];
            mockAudit.scrollbarWidth = { scrollbarWidth: 15, isOverlay: false };
            mockAudit.flexWrapAudit = [];
            mockAudit.overscrollBehavior = [];
            mockAudit.colorSpaceMeta = { cssColorSpace: "auto" };
            mockAudit.scrollbarColor = [];
            mockAudit.transformOrigin = [];
            mockAudit.outlineStyleAudit = [];
            mockAudit.selectAccentColor = { hasCustomAccent: false, items: [] };
            mockAudit.crossOriginCSS = { count: 0, sheets: [] };
            mockAudit.stickyStacking = { stickyCount: 1, conflicts: [] };
            mockAudit.inputTypeAudit = [];
            mockAudit.layoutShiftRisk = [];
            mockAudit.responsiveFontScale = { hasFluidType: false, items: [] };
            mockAudit.dataUriAssets = { inlineImages: 0, inlineFonts: 0, inlineSvgs: 0, estimatedSizeKB: 0 };
            mockAudit.printPageBreak = { hasPrintRules: false };
