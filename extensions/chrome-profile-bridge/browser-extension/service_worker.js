...
async function dispatch(action, params) {
  switch (action) {
    ...
    case "page.audit": {
      ...
      const results = await executeScriptTimed({
        ...
        func: () => {
          ...
          return {
            ...
            cssUnitAudit: (() => { const m = { px: 0, rem: 0, em: 0, percent: 0, vw: 0, vh: 0, svh: 0, dvh: 0, lvh: 0, vmin: 0, vmax: 0, ch: 0, ex: 0, ic: 0, lh: 0, rlh: 0, cap: 0, rcap: 0, fr: 0, pt: 0, pc: 0, cm: 0, mm: 0, in: 0, q: 0, deg: 0, rad: 0, turn: 0, s: 0, ms: 0 }; for (const sheet of document.styleSheets) { try { for (const rule of sheet.cssRules) { const text = rule.cssText || ""; for (const unit of Object.keys(m)) { const re = new RegExp(`\\b\d+${unit}\\b`, "g"); const c = (text.match(re) || []).length; if (c) m[unit] += c; } } } catch {} } return Object.entries(m).filter(([_,v]) => v > 0).sort((a,b) => b[1]-a[1]).map(([unit, count]) => ({ unit, count })); })(),
            namedColorUsage: (() => { const named = new Set(["red","blue","green","yellow","orange","purple","pink","black","white","gray","grey","silver","maroon","navy","teal","aqua","fuchsia","lime","olive","purple","maroon","transparent","currentcolor","aliceblue","antiquewhite","aquamarine","azure","beige","bisque","blanchedalmond","blueviolet","brown","burlywood","cadetblue","chartreuse","chocolate","coral","cornflowerblue","cornsilk","crimson","cyan","darkblue","darkcyan","darkgoldenrod","darkgray","darkgreen","darkkhaki","darkmagenta","darkolivegreen","darkorange","darkorchid","darkred","darksalmon","darkseagreen","darkslateblue","darkslategray","darkturquoise","darkviolet","deeppink","deepskyblue","dimgray","dodgerblue","firebrick","floralwhite","forestgreen","gainsboro","ghostwhite","gold","goldenrod","greenyellow","honeydew","hotpink","indianred","indigo","ivory","khaki","lavender","lavenderblush","lawngreen","lemonchiffon","lightblue","lightcoral","lightcyan","lightgoldenrodyellow","lightgray","lightgreen","lightpink","lightsalmon","lightseagreen","lightskyblue","lightslategray","lightsteelblue","lightyellow","limegreen","linen","magenta","mediumaquamarine","mediumblue","mediumorchid","mediumpurple","mediumseagreen","mediumslateblue","mediumspringgreen","mediumturquoise","mediumvioletred","midnightblue","mintcream","mistyrose","moccasin","navajowhite","oldlace","olivedrab","orangered","orchid","palegoldenrod","palegreen","paleturquoise","palevioletred","papayawhip","peachpuff","peru","plum","powderblue","rebeccapurple","rosybrown","royalblue","saddlebrown","salmon","sandybrown","seagreen","seashell","sienna","skyblue","slateblue","slategray","snow","springgreen","steelblue","tan","thistle","tomato","turquoise","violet","wheat","whitesmoke","yellowgreen"]); let count = 0; const found = []; for (const sheet of document.styleSheets) { try { for (const rule of sheet.cssRules) { const text = (rule.cssText || "").toLowerCase(); for (const n of named) { if (new RegExp(`\\b${n}\\b`).test(text) && !found.includes(n)) { found.push(n); count++; } } } } catch {} } return { count, colors: found.slice(0, 20) }; })(),
            intrinsicSizing: (() => { const m = { maxContent: 0, minContent: 0, fitContent: 0, stretch: 0, fitContentFunction: 0 }; for (const el of document.querySelectorAll("*")) { const s = getComputedStyle(el); for (const p of ["width","height","min-width","min-height","max-width","max-height","inline-size","block-size"]) { const v = s.getPropertyValue(p); if (/max-content/.test(v)) m.maxContent++; if (/min-content/.test(v)) m.minContent++; if (/fit-content/.test(v) && /fit-content\(/.test(v)) m.fitContentFunction++; else if (/fit-content/.test(v)) m.fitContent++; if (/stretch/.test(v)) m.stretch++; } } return m; })(),
            gridSpanAudit: (() => { const items = []; for (const el of document.querySelectorAll("*")) { const gc = el.style.gridColumn; const gr = el.style.gridRow; if ((gc && /span/.test(gc)) || (gr && /span/.test(gr))) { items.push({ tag: el.tagName, class: el.className?.toString?.()?.slice?.(0, 30) || undefined, gridColumn: gc || undefined, gridRow: gr || undefined }); if (items.length >= 15) break; } } return items; })(),
            atomicCss: (() => { const m = new Map(); for (const sheet of document.styleSheets) { try { for (const rule of sheet.cssRules) { if (rule.selectorText && /^\.\w+$/.test(rule.selectorText.trim()) && rule.style.length === 1) { const prop = rule.style[0]; m.set(prop, (m.get(prop) || 0) + 1); } } } catch {} } return { hasAtomic: m.size > 20, propertyCount: m.size, topProps: [...m.entries()].sort((a,b) => b[1]-a[1]).slice(0, 10).map(([prop, count]) => ({ prop, count })) }; })(),
            prefersColorSchemeDetailed: (() => { const darkVars = []; const lightVars = []; let hasDarkMedia = false; for (const sheet of document.styleSheets) { try { for (const rule of sheet.cssRules) { if (rule.media && /prefers-color-scheme:\s*dark/.test(rule.media.mediaText)) { hasDarkMedia = true; for (const inner of (rule.cssRules || [])) { if (inner.selectorText && /--/.test(inner.cssText)) { const vars = []; for (let i = 0; i < inner.style.length; i++) { const p = inner.style[i]; if (p.startsWith("--")) vars.push(p); } if (vars.length && darkVars.length < 20) darkVars.push({ selector: inner.selectorText.slice(0, 40), varCount: vars.length }); } } } if (rule.media && /prefers-color-scheme:\s*light/.test(rule.media.mediaText)) { for (const inner of (rule.cssRules || [])) { if (inner.selectorText && /--/.test(inner.cssText)) { const vars = []; for (let i = 0; i < inner.style.length; i++) { const p = inner.style[i]; if (p.startsWith("--")) vars.push(p); } if (vars.length && lightVars.length < 20) lightVars.push({ selector: inner.selectorText.slice(0, 40), varCount: vars.length }); } } } } } catch {} } return { hasDarkMedia, darkVarSets: darkVars.length, lightVarSets: lightVars.length, hasDataThemeAttr: !!document.querySelector("[data-theme], [data-color-scheme], .dark, .light"), issue: !hasDarkMedia && !document.querySelector("[data-theme]") ? "no dark mode detected (no prefers-color-scheme:dark, no [data-theme] toggle)" : undefined }; })(),
            scrollbarStylingFull: (() => { let webkitThumb = false, webkitTrack = false, firefoxWidth = false, firefoxColor = false; const props = []; for (const sheet of document.styleSheets) { try { for (const rule of sheet.cssRules) { if (rule.selectorText) { if (/::-webkit-scrollbar-thumb/.test(rule.selectorText)) webkitThumb = true; if (/::-webkit-scrollbar-track/.test(rule.selectorText)) webkitTrack = true; if (rule.cssText?.includes("scrollbar-width:")) { firefoxWidth = true; props.push("scrollbar-width"); } if (rule.cssText?.includes("scrollbar-color:")) { firefoxColor = true; props.push("scrollbar-color"); } } } } catch {} } return { webkitThumb, webkitTrack, firefoxWidth, firefoxColor, props, hasFullCustom: webkitThumb && webkitTrack && firefoxWidth, issue: webkitThumb && !firefoxWidth ? "webkit scrollbar styling without Firefox scrollbar-width — inconsistent cross-browser" : undefined }; })(),
            textFieldStyling: (() => { const items = []; for (const el of document.querySelectorAll("input[type=text],input[type=email],input[type=password],input[type=search],input[type=url],input[type=tel],textarea")) { const s = getComputedStyle(el); const ap = s.appearance !== "auto" && s.webkitAppearance !== "auto" ? s.appearance || s.webkitAppearance : undefined; if (ap === "none") { items.push({ tag: el.tagName, type: el.type, class: el.className?.toString?.()?.slice?.(0, 30) || undefined, appearance: "none", bg: s.backgroundColor !== "rgba(0, 0, 0, 0)" ? s.backgroundColor.slice(0, 20) : undefined, border: s.borderWidth !== "0px" ? `${s.borderWidth} ${s.borderStyle} ${s.borderColor.slice(0, 20)}` : undefined, borderRadius: s.borderRadius !== "0px" ? s.borderRadius : undefined }); if (items.length >= 10) break; } } return { customStyled: items.length, items }; })(),
            icLhUnits: (() => { const m = { ic: 0, lh: 0, rlh: 0, cap: 0, rcap: 0, rcap: 0, ric: 0 }; for (const sheet of document.styleSheets) { try { for (const rule of sheet.cssRules) { const text = rule.cssText || ""; for (const unit of Object.keys(m)) { const re = new RegExp(`\\b\d+${unit}\\b`); if (re.test(text)) m[unit]++; } } } catch {} } return Object.entries(m).filter(([_,v]) => v > 0).map(([unit, count]) => ({ unit, count })); })(),
            unknownAtRules: (() => { const known = new Set(["@import","@charset","@namespace","@media","@supports","@keyframes","@font-face","@page","@layer","@property","@container","@scope","@starting-style","@counter-style","@font-palette-values","@color-profile","@position-try"]); const unknown = []; for (const sheet of document.styleSheets) { try { for (const rule of sheet.cssRules) { const text = (rule.cssText || "").trim(); if (text.startsWith("@") && !known.has(text.split(" ")[0].split("(")[0])) { const name = text.match(/@[\w-]+/)?.[0]; if (name && !unknown.includes(name)) unknown.push(name); } } } catch {} } return unknown; })(),
            cssLayerOrder: (() => { const layers = []; for (const sheet of document.styleSheets) { try { for (const rule of sheet.cssRules) { if (rule.cssText?.startsWith("@layer")) { const names = rule.cssText.match(/@layer\s+([\w,\s-]+)/)?.[1]?.split(",").map(s => s.trim()) || []; for (const n of names) if (!layers.includes(n)) layers.push(n); } } } catch {} } return { count: layers.length, names: layers, hasOrder: layers.length > 0 }; })(),
            containerName: (() => { const items = []; for (const el of document.querySelectorAll("*")) { const s = getComputedStyle(el); const cn = s.containerName; if (cn && cn !== "none") { items.push({ tag: el.tagName, class: el.className?.toString?.()?.slice?.(0, 30) || undefined, containerName: cn, containerType: s.containerType }); if (items.length >= 10) break; } } return items; })(),
          };
        },
      ...
      if (audit.cssUnitAudit?.length) { lines.push("\n## CSS units"); for (const u of audit.cssUnitAudit.slice(0, 12)) lines.push(`  ${u.unit}: ${u.count}`); }
      if (audit.namedColorUsage?.count) { lines.push(`\n## Named colors: ${audit.namedColorUsage.count} used (${audit.namedColorUsage.colors.slice(0, 10).join(", ")})`); if (audit.namedColorUsage.count > 10) lines.push("  NOTE: many named colors — consider hex/oklch for precision and consistency"); }
      if (audit.intrinsicSizing) { const active = Object.entries(audit.intrinsicSizing).filter(([_,v]) => v > 0); if (active.length) { lines.push("\n## Intrinsic sizing"); for (const [k, v] of active) lines.push(`  ${k}: ${v}`); } }
      if (audit.gridSpanAudit?.length) { lines.push(`\n## Grid span (${audit.gridSpanAudit.length})`); for (const g of audit.gridSpanAudit.slice(0, 8)) lines.push(`  ${g.tag}${g.class ? `.${g.class}` : ""} — col:${g.gridColumn || ""} row:${g.gridRow || ""}`); }
      if (audit.atomicCss?.hasAtomic) { lines.push(`\n## Atomic/utility CSS: ${audit.atomicCss.propertyCount} single-prop classes`); for (const p of audit.atomicCss.topProps.slice(0, 5)) lines.push(`  .${p.prop}: ${p.count} rules`); }
      if (audit.prefersColorSchemeDetailed) { const d = audit.prefersColorSchemeDetailed; lines.push(`\n## Dark mode: ${d.hasDarkMedia ? "has prefers-color-scheme:dark" : "no media query"}${d.hasDataThemeAttr ? ", has [data-theme] toggle" : ""} (${d.darkVarSets} dark var sets, ${d.lightVarSets} light var sets)`); if (d.issue) lines.push(`  ${d.issue}`); }
      if (audit.scrollbarStylingFull?.issue) { lines.push(`\n## Scrollbar styling: webkit thumb:${audit.scrollbarStylingFull.webkitThumb} track:${audit.scrollbarStylingFull.webkitTrack}, firefox width:${audit.scrollbarStylingFull.firefoxWidth} color:${audit.scrollbarStylingFull.firefoxColor}`); lines.push(`  ${audit.scrollbarStylingFull.issue}`); }
      if (audit.textFieldStyling?.customStyled) { lines.push(`\n## Text field styling: ${audit.textFieldStyling.customStyled} with appearance:none`); for (const t of audit.textFieldStyling.items.slice(0, 5)) lines.push(`  ${t.tag}[${t.type}] — ${t.border || "no border"} r:${t.borderRadius || "0"}`); }
      if (audit.icLhUnits?.length) { lines.push("\n## Modern font-relative units"); for (const u of audit.icLhUnits) lines.push(`  ${u.unit}: ${u.count} uses`); }
      if (audit.unknownAtRules?.length) { lines.push(`\n## Unknown @-rules: ${audit.unknownAtRules.join(", ")}`); }
      if (audit.cssLayerOrder?.hasOrder) { lines.push(`\n## @layer order: ${audit.cssLayerOrder.count} layers (${audit.cssLayerOrder.names.join(", ")})`); }
      if (audit.containerName?.length) { lines.push(`\n## Named containers (${audit.containerName.length})`); for (const c of audit.containerName.slice(0, 5)) lines.push(`  ${c.tag} — name:${c.containerName} type:${c.containerType}`); }
